...
function computeStats(times, bytes) {
  const sorted = [...times].sort((a, b) => a - b)
  const median = sorted[Math.floor(sorted.length / 2)]
  const mean = times.reduce((a, b) => a + b, 0) / times.length
  // Guard against 0ms median (shouldn't happen with batching, but just in case)
  const throughputMBps = median > 0 ? (bytes / (1024 * 1024)) / (median / 1000) : 0
  const opsPerSec = median > 0 ? 1000 / median : 0

  return { median, mean, throughputMBps, opsPerSec, samples: times.length }
}
...
export async function runBenchmark(onProgress) {
  ...
  for (const size of SIZES) {
    ...
    sizeResults.algorithms.blake3 = computeStats(blake3Times, size.bytes)
    ...
    sizeResults.algorithms.sha256wasm = computeStats(sha256WasmTimes, size.bytes)
    ...
    if (hasWebCrypto) {
      ...
      sizeResults.algorithms.sha256crypto = computeStats(sha256CryptoTimes, size.bytes)
    }
